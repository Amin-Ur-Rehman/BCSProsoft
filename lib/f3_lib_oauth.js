...
OAuth.prototype.getBaseString = function (request, oauth_data) {
    ...
    return request.method.toUpperCase() + '&' + this.percentEncode(this.getBaseUrl(url)) + '&' +
...
OAuth.prototype.getParameterString = function (request, oauth_data) {
    ...
    var base_string_data =
      this.sortObject(this.percentEncodeData(
        this.mergeObject(oauth_data, this.mergeObject(request.data, this.deParamUrl(url)))));
...
/**
 * Generates Signed URL for the give data
 * @param obj
 * @param num_prefix
 * @param temp_key
 * @returns {string}
 */
OAuth.prototype.generateQueryParam = function (obj, num_prefix, temp_key) {
    var output_string = []
    ...
    Object.keys(obj).forEach(function (val) {
        ...
        if (typeof obj[val] === 'object') {
            var query = OAuth.prototype.generateQueryParam(obj[val], null, key);
            output_string.push(query);
        }
        else {
            var value = encodeURIComponent(obj[val].toString().replace(/[!'()*]/g));
