 ...
 */
/**
 * ItemImportManager class that has the actual functionality of Item Import.
 * All business logic will be encapsulated in this class.
 */
var ItemImportManager = (function () {
    return {
            ...
            try {
                ...
                try {
                    for (var i in externalSystemArr) {
                        ...
                        if (!FeatureVerification.isPermitted(Features.IMPORT_ITEM_FROM_EXTERNAL_SYSTEM, ConnectorConstants.CurrentStore.permissions)) {
                            Utility.logEmergency('FEATURE PERMISSION', Features.IMPORT_ITEM_FROM_EXTERNAL_SYSTEM + ' NOT ALLOWED');
                            continue;
                        }
                    ...
                    Utility.logException('ItemImportManager.scheduled - Iterating Items', e);
                }
                Utility.logDebug('Ends', 'Ends...');
            }
            catch (e) {
                Utility.logException('ItemImportManager.scheduled', e);
            }
        },
            ...
            if (!!records && records.length) {
                for (var i = 0; i < records.length; i++) {
                    try {
                        ...
                        // commented by allzohu
                        //ItemImportLibrary.markStatus(itemRecordType, itemInternalId, e.toString());
                    }
                }
            }
        },
 ...
 * @returns {Void}
 */
function ItemImportManagerScheduled(type) {
    return ItemImportManager.scheduled(type);
}