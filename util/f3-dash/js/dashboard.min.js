...
function MasterController(e, $http, f3Store, $scope, f3Utility) {
        ...
        loadData: function (options) {
            ...
            $http.get(apiUrl)
                .success(function (response) {
                    ...
                    var chart = new google.charts.Bar(document.getElementById('customers_top_sales_bar_chart'));
                    chart.draw(view, barChartOptions);
    ...
    viewModel.graphs.dataSyncFrequency = {
        ...
        loadData: function () {
            ...
            var dashboard = new google.visualization.Dashboard(document.getElementById('data_sync_frequency_dashboard'));

            var guageSelector = new google.visualization.ControlWrapper({
                'controlType': 'CategoryFilter',
                'containerId': 'data_sync_frequency_controller',
                'options': {
            ...
            var gaugeChart = new google.visualization.ChartWrapper({
                'chartType': 'Gauge',
                'containerId': 'data_sync_frequency',
                'options': {
                    width: 800,
                    height: 240,
                    minorTicks: 5,
            ...
            dashboard.bind(guageSelector, gaugeChart);
            dashboard.draw(data);
        }
    };

    viewModel.graphs.soCompareMonthly = {
        loading: false,
        data: [],
        hasData: !!this.data,
        loadData: function () {
            viewModel.graphs.soCompareMonthly.loading = false;
            viewModel.graphs.soCompareMonthly.hasData = true;
        }
    };


    viewModel.graphs.soStatus = {
        loading: false,
        data: [],
        hasData: !!this.data,
        loadData: function () {
            viewModel.graphs.soStatus.loading = false;
            viewModel.graphs.soStatus.hasData = true;

            var sortedData = _.groupBy(viewModel.graphs.soStatus.data, function(obj){
            ...
            data.addRows(_.keys(sortedData).length);

            for(var i = 0; i < _.keys(sortedData).length; i++) {
                data.setCell(i, 0, _.keys(sortedData)[i]);
                data.setCell(i, 1, sortedData[_.keys(sortedData)[i]].length);
            }
            ...
            var barChart = new google.charts.Bar(document.getElementById('data_so_status_bar'));
            barChart.draw(data, barChartOptions);

            var pieChart = new google.visualization.PieChart(document.getElementById('data_so_status_pie'));
    ...
    viewModel.failedSalesOrdersImported = [],
    ...
    var apiUrl = location.href.replace(location.hash, '') + '&method=getSalesOrders&store_id=' + f3Store.id;
    $http.get(apiUrl)
        .success(function (response) {
            viewModel.graphs.soStatus.data = response;
            viewModel.graphs.soStatus.loadData();
        });
    ...
    apiUrl = location.href.replace(location.hash, '') + '&method=getFailedSalesOrdersImported&store_id=' + f3Store.id;
    $http.get(apiUrl)
        .success(function (response) {
            viewModel.failedSalesOrdersImported = response;
        });
}
